...
TG_PROXY = "http://150.241.245.131:8080"
...
DOMAINS = [
    ...
    "known", "unknown", "slark", "unravelesq",
]
...
async def setup_telegram(session):
    ...
    try:
        async with session.get(
            ...
            print(f"[TG] Проверка OK через прокси {TG_PROXY}")
...
async def fetch_domain(session, domain, token):
    ...
    try:
        ...
        print(f"[NET] {domain}: timeout")
        return {"ok": False, "error": "timeout"}

    except aiohttp.ClientError as e:
        print(f"[NET] {domain}: network ({e})")
        return {"ok": False, "error": "network"}

    except Exception as e:
        print(f"[NET] {domain}: exception ({e})")
        return {"ok": False, "error": "exception"}

    if "error" in data:
        code = data["error"].get("error_code")
        msg = data["error"].get("error_msg", "")
        print(f"[VK ERROR] {domain}: код {code} — {msg}")
    ...
    async def check_one(index, domain):
        ...
        async with semaphore:
            ...
            if last_id is not None and current_id is None:
                ...
                print(f"[RELEASE] {domain}: {owner_number(last_id)} → FREE")
            ...
            if last_id is None and current_id is not None:
                ...
                url = owner_url(new_type, current_id)

                print(f"[SWAP] {domain}: FREE → {url}")
...
async def main():
    ...
    print(f"[TG] Прокси для Telegram: {TG_PROXY}")
    ...
    async with aiohttp.ClientSession(
        ...
        while True:
            try:
                ...
                print(f"[RUN_PASS ERROR] {type(e).__name__}: {e}")
